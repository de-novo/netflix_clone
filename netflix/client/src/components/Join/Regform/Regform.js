...
import Input100 from "../../Atom/Input_100";
...
function Regform() {
    ...
    const isUser = (data) => {
        ...
        if (data.isUser&&data.accessToken) {
        ...
        <div className="regform">
            ...
            <main className="regform-main">
                <div className="regform-container">
                    ...
                    <Input100
                        attr={{ id: "signup_email", name: "Email" }}
                        onChange={onChange}
                        inputs={inputs?inputs:''}
                        label={"이메일 주소"}
                        inputStyle={{ fontSize: "1.8rem", border: "1px solid #8c8c8c", height: "6rem", margin: "0.5rem 0" }}
                    ></Input100>
                    <Input100
                        attr={{ id: "signup_pw", name: "password" }}
                        onChange={onChange}
                        inputs={inputs?inputs:''}
                        label={"비밀번호를 추가하세요"}
                        inputStyle={{ fontSize: "1.8rem", border: "1px solid #8c8c8c", height: "6rem", margin: "0.5rem 0" }}
                    ...
                    <div class="checkbox-container">
                        ...
                        <span className="medium greycolor">
                            예, 저는 <Link to="/">개인정보 처리방침</Link>에 따른 개인정보 수집 및 활용에 동의합니다.
                        </span>{" "}
                    </div>
                    <div class="checkbox-container">
                    ...
                    <Button100 content={"동의하고 계속"} onSubmit={onSubmit} style={{ fontSize: "2.4rem" }}></Button100>
                </div>
            </main>
        </div>
    );
